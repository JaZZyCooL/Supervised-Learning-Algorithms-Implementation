...
knn = KNeighborsClassifier(n_neighbors=30, p=2, weights=('distance'), algorithm = 'auto')
...
#n_neighbors=8, p=1, weights=('distance'), algorithm = 'auto' - learning curve 1
...
#     knn = KNeighborsClassifier(n_neighbors=30, p=p[i], weights='distance', algorithm='auto')
#     score.append(cross_val_score(knn, X, Y, cv=10, verbose=1, n_jobs=1).mean())

# plt.plot(p, score, color='green', marker='+', markersize=5, linestyle='--', label='Validation Accuracy')
